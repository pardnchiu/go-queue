...
> 輕量的 Golang 優先級佇列，支援有界併發、優先級提升和優雅關閉。最大化硬體資源利用並防止系統過載<br>
...
### 優先級佇列
使用最小堆實現的四級優先級系統（Immediate > High > Normal > Low）。高優先級任務優先執行，相同優先級的任務按 FIFO 順序處理。
...
| Normal | 2 | timeout（15-120s）| 背景任務 |
| Low | 3 | timeout × 2（15-120s）| 清理、分析 |
